#!/usr/bin/env python3
"""Benchmark the hard-filters + simulability pipeline against the live RCSB API.
...
and the hard-filters search itself runs server-side (never scans the whole archive
...
def benchmark_one_size(n: int) -> list[StageTiming]:
    """Run the full hard-filters + simulability pipeline once for ``n`` requested candidates."""
...
def main() -> None:
    ...
    print(f"Benchmarking hard-filters + simulability pipeline for sizes: {args.sizes}")
