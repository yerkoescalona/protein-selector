...
class SimulabilityResult:
    """Outcome of the simulability size/resolution gate for one candidate."""
...
def check_size_and_resolution(
    ...
    """Simulability gate: residue-count window + a tightened resolution ceiling.

    The hard filters already filter on a coarser resolution/atom-count ceiling (see
    ``candidates.build_hard_filters_query``); this catches survivors that cleared the
    hard filters' SQL-side filter but are still too large, too small, or too low-resolution
...
def check_completeness(
    ...
    """Simulability gate: fraction of unmodeled (missing/disordered) residues.

    Uses ``n_modeled_residues``/``n_unmodeled_residues`` -- fields the hard filters already
...
def check_oligomeric_state(
    ...
    """Simulability gate: reject overly complex oligomers, if a ceiling is set.
...
def check_non_standard_residues(
    ...
    """Simulability gate: reject non-standard residues, unless explicitly allowed.
...
def check_full_simulability(
    ...
    """Full simulability gate: composes all four checks into one ``SimulabilityResult``.
    ...
    None``: that field is data the hard filters' OWN fetch should always provide, so its
...
def filter_simulable(
    ...
    """Apply the simulability size/resolution gate to a batch of hard-filters survivors.
