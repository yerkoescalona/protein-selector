"""Tests for protein_selector.docking.native_ligand (PLAN.md §17a/§18).
...
class TestPrepareLigandPdbqt:
    def test_missing_binary_raises_file_not_found_with_install_hint(self, tmp_path):
        ...
        with patch("protein_selector.docking.native_ligand.subprocess.run", side_effect=FileNotFoundError):
            try:
    ...
    def test_empty_output_file_raises_runtime_error(self, tmp_path):
        ...
        with patch(
            "protein_selector.docking.native_ligand.subprocess.run",
    ...
    def test_real_nonempty_output_is_returned(self, tmp_path):
        ...
        with patch(
            "protein_selector.docking.native_ligand.subprocess.run", side_effect=_fake_obabel
        ):
