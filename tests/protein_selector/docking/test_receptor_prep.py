"""Tests for protein_selector.docking.receptor_prep.
...
class TestPrepareReceptorPdbqt:
    def test_missing_binary_raises_file_not_found_with_install_hint(self, tmp_path):
        ...
        with patch(
            "protein_selector.docking.receptor_prep.subprocess.run", side_effect=FileNotFoundError
        ):
    ...
    def test_missing_output_file_raises_runtime_error(self, tmp_path):
        ...
        with patch(
            "protein_selector.docking.receptor_prep.subprocess.run", return_value=fake_result
        ):
    ...
    def test_real_output_file_returns_dest_path(self, tmp_path):
        ...
        with patch("protein_selector.docking.receptor_prep.subprocess.run", side_effect=fake_run):
